...
class GenomeQC:
    """Checks whether the completeness&contamination score is within user-defined range."""
    def __init__(self, checkm_input_folder, config_file, output_file):
        self.checkm_input_folder = Path(checkm_input_folder)
    ...
    def run(self):
        """Assess the checkm output for each sample folder in input folder dir.
            Input: path to checkm output folder
            Output: CSV file containing isolates that fall outside the defined range."""
        failures = []
        # for each sample folder in input folder direcotry
        for sample_dir in self.checkm_input_folder.iterdir():
